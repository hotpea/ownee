class EmployeeController < ApplicationController
  ...
  def save
    ...
    manager = params[:id] ? Department.exists?(employee_id: params[:id]) : false

    if not @employee.birth
      redirect_to request.referrer, flash: {error: 'Preencha a data de nascimento!!!'}
    elsif Time.now.year - 18 < @employee.birth.year
